...
func ValidateConfig(cfg *Config) error {
	...
	if cfg.AutoBioEnabled {
		...
		if cfg.AutoBioInitialAnalysisPrompt != "" && (!strings.Contains(cfg.AutoBioInitialAnalysisPrompt, "%s")) {
			log.Printf("[WARN] AUTO_BIO_INITIAL_ANALYSIS_PROMPT не содержит плейсхолдеры %%s для имени и сообщений.")
		}
		if cfg.AutoBioUpdatePrompt != "" && (!strings.Contains(cfg.AutoBioUpdatePrompt, "%s") || strings.Count(cfg.AutoBioUpdatePrompt, "%s") < 3) {
			log.Printf("[WARN] AUTO_BIO_UPDATE_PROMPT не содержит плейсхолдеры %%s для имени, старого био и новых сообщений.")
		}
	}
	...
	if cfg.ModPurgeDuration <= 0 {
		return fmt.Errorf("MOD_PURGE_TIME_MIN (%v) должен быть > 0 (например '30s' или '1m')", cfg.ModPurgeDuration)
	}
	if cfg.ModPurgeDelay < 0 {
		return fmt.Errorf("MOD_PURGE_DELAY_DURATION (%v) должен быть >= 0", cfg.ModPurgeDelay)
	}
