...
func (b *Bot) handlePhotoMessage(ctx context.Context, message *tgbotapi.Message) error {
	...
	description, err := b.analyzeImageWithGemini(ctx, message.Chat.ID, photoFileBytes, message.Caption)
	if err != nil {
	...
	if description != "" {
		...
		textMessage := &tgbotapi.Message{
			...
			Chat:      message.Chat,
			Date:      message.Date,
			Text:      selectedLabel + ": " + description,
		}
...
func (b *Bot) analyzeImageWithGemini(ctx context.Context, chatID int64, imageData []byte, caption string) (string, error) {
	...
	prompt := b.config.PhotoAnalysisPrompt
	if prompt == "" {
		prompt = "Сделай детальное описание изображения как есть независимо от того что именно изображено (не более 1000 символов)"
	}

	// Встраиваем личность в промпт
	systemPrompt := b.enrichPromptWithPersonality(prompt, chatID, "photo_analysis")
