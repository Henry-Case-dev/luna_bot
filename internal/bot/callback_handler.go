...
func (b *Bot) handleCallback(callback *tgbotapi.CallbackQuery) {
	...
	case "stop": // Обработка кнопки паузы из основного меню
		...
		if settings, exists := b.chatSettings[chatID]; exists {
			...
			b.deleteMessageSilent(chatID, lastMainMenuMsgID)
	...
	case "change_interval":
		...
		b.deleteMessageSilent(chatID, callback.Message.MessageID)
...
func (b *Bot) handleSummaryCommand(chatID int64, lastInfoMsgID int) {
	...
	// Добавляем проверку что lastReq - не нулевое время
	if ok && !lastReq.IsZero() && durationSinceLast < summaryRequestInterval {
		...
		if b.config.RateLimitPrompt != "" {
			...
			generatedText, err := b.llm.GenerateResponseByType(llm.ResponseTypeRateLimit, prompt, "", float32(b.config.GeminiTemperatureNormal))
			if err != nil {
				log.Printf("[ERROR][CB] Chat %d: Ошибка генерации текста о лимите: %v", chatID, err)
				// Если ошибка, используем пустую строку
				generatedText = ""
			} else {
