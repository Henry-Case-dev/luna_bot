...
func (c *Client) GenerateResponse(systemPrompt string, history []*tgbotapi.Message, lastMessage *tgbotapi.Message, temperature float32) (string, error) {
	if c.debug {
		...
		log.Printf("[DEBUG] DeepSeek Запрос: Модель %s, Температура %.2f", c.modelName, temperature)
	}
...
func (c *Client) GenerateArbitraryResponse(systemPrompt string, contextText string, temperature float32) (string, error) {
	if c.debug {
		...
		log.Printf("[DEBUG] DeepSeek Запрос (Arbitrary): Модель %s, Температура %.2f", c.modelName, temperature)
	}
...
func (c *Client) GenerateResponseFromTextContext(systemPrompt string, contextText string, temperature float32) (string, error) {
	if c.debug {
		...
		log.Printf("[DEBUG] DeepSeek Запрос (Text Context): Модель %s, Температура %.2f", c.modelName, temperature)
	}
...
// Для DeepSeek клиента используется GenerateArbitraryResponse независимо от типа ответа.
func (c *Client) GenerateResponseByType(responseType llm.ResponseType, systemPrompt string, contextText string, temperature float32) (string, error) {
	if c.debug {
		log.Printf("[DEBUG] DeepSeek: Генерация ответа типа %s с температурой %f", responseType, temperature)
	}

	// Для DeepSeek используем стандартный метод GenerateArbitraryResponse
...
func (c *Client) GenerateImageWithEdit(ctx context.Context, baseImageData []byte, editPrompt string) ([]byte, error) {
	if c.debug {
		log.Printf("[DEBUG] DeepSeek не поддерживает генерацию изображений.")
	}
	return nil, errors.New("DeepSeek не поддерживает генерацию изображений")
}

// GenerateAudio — заглушка для AudioGenerator. DeepSeek не поддерживает TTS.
// Будет удалена в CLN-03 после полного перехода на capability-интерфейсы.
func (c *Client) GenerateAudio(text string, params llm.AudioParams) ([]byte, error) {
	if c.debug {
		log.Printf("[DEBUG] DeepSeek не поддерживает аудиогенерацию.")
	}
	return nil, errors.New("DeepSeek не поддерживает аудиогенерацию")
}

// Info возвращает метаинформацию о провайдере DeepSeek.
...
// Compile-time interface satisfaction checks for DeepSeek.
var (
